...
public class CloudUpdateServerPacket implements Packet {
    ...
    public void read(ByteBuf buf, Channel channel) {
        ...
        for(UUID uuid : player) {
            ...
            Cloud.getINSTANCE().getConnectionsToServer().forEach(c ->{ if(!c.equals(channel)) {
                c.writeAndFlush(this);
                        System.out.println("sent server update packet to: " + channel);
            }
            }
            );
            if(!channel.equals(Cloud.getINSTANCE().getConnectionToProxyWrapper())) {
            Cloud.getINSTANCE().getConnectionToProxyWrapper().writeAndFlush(this);
                System.out.println("sent server update packet to proxy");}
