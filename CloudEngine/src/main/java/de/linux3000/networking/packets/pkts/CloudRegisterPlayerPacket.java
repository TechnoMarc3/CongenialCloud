...
public class CloudRegisterPlayerPacket implements Packet {
    ...
    public void read(ByteBuf buf) {
        ...
        CloudApi.getINSTANCE().getPlayerManager().addPlayerToCloud(new CloudPlayer(name, uuid));

        Cloud.getINSTANCE().getConnectionsToServer().forEach(channel -> {channel.writeAndFlush(this);
            System.out.println("sent register player packet to channel: " + channel);}
        );
    }
